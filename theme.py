...
# ─────────────────────────────────────────────────────────────────────────────
# Aurora night-sky theme — definitive fix
#
# ROOT CAUSES fixed vs previous versions:
#
# 1. z-index:-1 TRAP
#    Streamlit's .stApp has a CSS transform / overflow property that creates a
#    NEW stacking context.  Inside a stacking context, z-index:-1 goes BEHIND
#    that stacking context's background — meaning the canvas is behind the dark
#    background of .stApp, invisible.  Fix: use z-index:0 on canvas and push
#    all Streamlit layers to z-index:1+, OR (better) neutralise .stApp's
#    stacking context entirely by making sure it has no transform/will-change.
#
# 2. SCROLL CONTAINER
#    In Streamlit ≥ 1.28 the scroll happens inside [data-testid="stMain"] /
#    .stApp, NOT on window.  position:fixed is relative to the viewport which
#    is correct — BUT if Streamlit sets overflow:hidden on <html>/<body> then
#    scrolling happens on an inner div.  The canvas appears fixed to the top
#    of that inner div (which is the full page) so it looks "stuck at the top".
#    Fix: force html,body to overflow:auto and let the window scroll.
#
# 3. ctx.roundRect not available in all Chromium builds → polyfill.
#
# 4. Canvas survival across Streamlit React re-renders → MutationObserver.
# ─────────────────────────────────────────────────────────────────────────────
...
/* ── SCROLL FIX: let the page scroll on the window, not an inner div ── */
html {
...
/* ── STACKING FIX: strip the stacking context from Streamlit wrappers ── */
/* We make every wrapper transparent AND remove transforms/overflow that      */
/* create stacking contexts trapping z-index:-1 children.                    */
.stApp {
  background: transparent !important;
  /* Remove properties that create a stacking context */
  transform: none !important;
  will-change: auto !important;
  /* Don't set position here — let it be whatever Streamlit sets */
}
...
(function () {
  ...
  if (!CanvasRenderingContext2D.prototype.roundRect) {
    CanvasRenderingContext2D.prototype.roundRect = function(x,y,w,h,r) {
      ...
      this.moveTo(x+rad,y);
      this.lineTo(x+w-rad,y); this.quadraticCurveTo(x+w,y,x+w,y+rad);
      ...
      this.lineTo(x,y+rad); this.quadraticCurveTo(x,y,x+rad,y);
      this.closePath(); return this;
    };
  }

  /* ── Inject scroll + stacking-context fix styles ── */
  var s = document.createElement('style');
  s.id = '__aurora_fix__';
  s.textContent =
    /* Let the page scroll on window, not an inner div */
    'html{overflow:auto!important;height:auto!important;background:#04080f!important}' +
    'body{overflow:auto!important;height:auto!important;min-height:100vh!important;background:transparent!important}' +
    /* Strip stacking contexts from every Streamlit wrapper so z-index:-1 works */
    '.stApp{background:transparent!important;transform:none!important;will-change:auto!important;overflow:visible!important;}' +
    '[data-testid="stAppViewContainer"],[data-testid="stAppViewBlockContainer"],' +
    '[data-testid="stMain"],.main,[data-testid="block-container"],.block-container,' +
    '[data-testid="stVerticalBlock"]{background:transparent!important;transform:none!important;will-change:auto!important;}';
  document.head.appendChild(s);

  /* ── Create & mount the background canvas ── */
  function mkCanvas() {
    var c = document.createElement('canvas');
    c.id = '__night_sky__';
    /* Fixed to viewport, behind everything (z-index:-1).
       Because we stripped stacking contexts above, -1 is now relative to
       the root stacking context and truly goes behind all page content.  */
  ...
  /* Re-mount if Streamlit's React re-render removes the canvas */
  new MutationObserver(function() {
    if (!document.getElementById('__night_sky__')) {
      cv = mkCanvas(); ctx = cv.getContext('2d'); resize();
    }
  }).observe(document.body, { childList: true });

  /* ── Scene data ── */
  var stars = [];
  for (var i=0; i<260; i++) stars.push({
